...
export class TasksService {
   ...
   *
   * Field keys are mapped to their engine variable names via each task's form
   * definition (field.variable || field.name); the raw field key is kept as an
   * alias too, so forms that share field names but not variable names still match.
   */
  private async getInstanceVariables(instanceId: string): Promise<Record<string, any>> {
    const instanceTasks = await this.prisma.task.findMany({
      where: { processInstanceId: instanceId },
      include: {
  ...
  async complete(id: string, dto: CompleteTaskDto, userId: string, role?: string) {
    ...
    if (dto.data && Object.keys(dto.data).length > 0) {
      ...
      const stamped = await this.files.stampFromSubmissionData(dto.data, id, task.processInstanceId);
      if (stamped > 0) {
