...
import type {
  BpmnStartInstanceOptions,
  BpmnResumeInstanceOptions,
  EngineCallbacks,
  UserTaskDefinition,
...
export class BpmnEngineService {
   ...
   */
  async bpmnStartInstance(opts: BpmnStartInstanceOptions): Promise<void> {
   ...
   */
  async bpmnResumeInstance(opts: BpmnResumeInstanceOptions): Promise<void> {
    const { Engine } = await this.getEngineModule();
    const engine = Engine({
      name: `instance-${opts.instanceId}`,
      source: opts.bpmnXml,
    ...
    engine.broker.subscribeTmp(
      ...
      async () => {
        ...
        try {
          ...
          this.logger.error(`onEnd callback failed for ${instanceId}: ${(err as Error).message}`);
        }
      },
      { noAck: true },
    );
    ...
    engine.broker.subscribeTmp(
      ...
      async (_routingKey: string, msg: any) => {
        ...
        try {
          ...
          this.logger.error(`onError callback failed for ${instanceId}: ${(cbErr as Error).message}`);
        }
      },
      { noAck: true },
    );
  ...
  private createListener(instanceId: string, callbacks: EngineCallbacks): EventEmitter {
    ...
    listener.on('wait', async (api: any) => {
      ...
      try {
        ...
        this.logger.error(`onUserTask callback failed for ${instanceId}: ${(err as Error).message}`);
      }
   ...
   */
  async bpmnSignalTask(instanceId: string, executionId: string, data: unknown): Promise<void> {
    const key = `${instanceId}:${executionId}`;
    ...
    if (api.environment && data && typeof data === 'object') {
      try {
        ...
        this.logger.log(
          `Merged form data into variables for instance ${instanceId}: ${JSON.stringify(Object.keys(data as object))}`,
        );
      } catch (err: unknown) {
        this.logger.warn(`Failed to merge variables for ${instanceId}: ${(err as Error).message}`);
      }
    }
   ...
   */
  async bpmnTerminateInstance(instanceId: string): Promise<void> {
    const engine = this.engines.get(instanceId);
    if (engine) {
      try {
        await engine.stop();
  ...
  private async persistState(instanceId: string, callbacks: EngineCallbacks): Promise<void> {
    ...
    try {
      ...
      this.logger.warn(`Failed to persist state for ${instanceId}: ${(err as Error).message}`);
    }
  }
