...
export class ReportsService {
  ...
  async update(id: string, dto: UpdateReportDto) {
    ...
    const columns = dto.columns !== undefined ? this.parseColumns(dto.columns) : this.parseJsonArray(existing.columns);
    const filters = dto.filters !== undefined ? this.parseFilters(dto.filters) : this.parseJsonArray(existing.filters);
  ...
  async remove(id: string) {
    const existing = await this.prisma.reportDefinition.findUnique({ where: { id }, select: { id: true } });
    if (!existing) throw new NotFoundException(`Report ${id} not found`);
  ...
  /**
   * Everything an admin can pick as a report column for a process:
   *  - the fixed instance fields, and
   *  - the process VARIABLES = declared ProcessVariables ∪ form-field
   *    variables (field.variable || field.name), enriched with select
   *    options (category items with Persian labels, inline fallback —
   *    exactly like the runtime form resolves them).
   */
  async getFieldCatalog(processId: string) {
   ...
   * instance's variables (same semantics as TasksService's prefill chain),
  ...
  private async runReport(processId: string, columns: ReportColumnDto[], filters: ReportFilterDto[]) {
    ...
    for (const inst of instances) {
      const vars = this.mergeInstanceVariables(inst.tasks);
      // Filters are evaluated on the merged variables (independent of which
      // VARIABLE columns the report selected), then the row is projected.
  ...
  // -------------------------------------------------------------------------
  // Instance variable merge — mirrors TasksService.getInstanceVariables
  ...
  private mergeInstanceVariables(tasks: any[]): Record<string, any> {
  ...
  private computeInstanceField(fieldKey: string, inst: any): any {
    switch (fieldKey) {
      case 'status':
        return inst.status;
      case 'startedBy':
  ...
  private matchFilters(filters: ReportFilterDto[], inst: any, vars: Record<string, any>): boolean {
    for (const f of filters) {
      if (f.type === 'STATUS') {
        ...
        const actual = raw === null || raw === undefined ? '' : typeof raw === 'object' ? JSON.stringify(raw) : String(raw);
  ...
  private async assertProcess(processId: string) {
    const process = await this.prisma.process.findUnique({ where: { id: processId }, select: { id: true } });
  ...
  private async loadVariableCatalog(processId: string): Promise<Map<string, CatalogVariable>> {
    const declared = await this.prisma.processVariable.findMany({
      where: { processId },
      orderBy: { createdAt: 'asc' },
    });
  ...
  private async validateColumns(processId: string, columns: ReportColumnDto[]) {
    for (const col of columns) {
  ...
  private validateFilters(filters: ReportFilterDto[]) {
    for (const f of filters) {
      if (f.type === 'STATUS') {
