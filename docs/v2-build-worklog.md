...
---
Task ID: sidebar-roles-update
Agent: Super Z (main agent)
Task: Sidebar renames (بایگانی فرآیندها admin-only, کارتابل) + new SENIOR_EXPERT role (کارشناس ارشد), rename کاربر → کارشناس

Work Log:
- Backend: added SENIOR_EXPERT to users DTO UserRole enum (user.dto.ts) and Prisma schema enum; SQLite role column is plain TEXT (no CHECK) → zero-impact change, client regenerated; RolesGuard untouched (SENIOR_EXPERT has USER-level access; @Roles('ADMIN') endpoints stay admin-only)
- Frontend i18n: myTasks «وظایف من»→«کارتابل»; instances «نمونه‌های فرآیند»→«بایگانی فرآیندها» (user asked «بایگانی فرآیند ها» — normalized to standard orthography); USER «کاربر»→«کارشناس»; added SENIOR_EXPERT «کارشناس ارشد»; new roleLabel() helper + roleLabels map; statusColors.SENIOR_EXPERT = indigo tint
- Sidebar + Ctrl+K command palette: /instances nav item now show:isAdmin (admin-only); app-bar + sidebar user box use roleLabel()
- Dashboard quick actions: archive card wrapped in isAdmin (non-admin sees 3 cards: کارتابل / شروع نمونه / تعریف فرآیندها)
- users-view: role filter + create/edit dialog now ADMIN/SENIOR_EXPERT/USER; MUI chip sx for SENIOR_EXPERT (color-mix primary 12%); role chips use roleLabel() with USER fallback
- Environment: sandbox re-created from zip → promoted frontend to project root + backend to mini-services/bpms-backend; seeded DB (4 demo users, leave process)
- Watcher budget fix: nest --watch consumed all 8192 inotify watches → Turbopack "OS file watch limit reached" panic loop on frontend spawn; switched backend to plain `nest start` (no watch) → FrontendSupervisor spawns/keeps frontend reliably
- tsc exclude: added "bpms-v2" (stale reference copy inside project) to root tsconfig exclude, matching the original workspace's stale-copy exclusions
- Verification: browser walkthrough admin + john (sidebar labels/visibility, palette filtering, dashboard cards, users table کارشناس/کارشناس ارشد chips light+dark, created sara@bpms.local/user123 as کارشناس ارشد via dialog → API confirms role=SENIOR_EXPERT); console ZERO errors; E2E 6/6 GREEN (persian-process, dashboard, condition-validation 9/9, versioning, file-upload, task-starter); tsc: frontend 18 = baseline set (only the documented api.ts union-print cosmetic shift), backend 0; lint clean on all touched files
- Screenshots → download/bpms-sidebar-changes/ (01 john dashboard, 02 admin dashboard, 03 users light, 04 users dark)

Stage Summary:
- «بایگانی فرآیندها» is admin-only in sidebar + palette + dashboard quick actions (page route still reachable for non-admin deep links — task detail instance links keep working)
- «کارتابل» replaces «وظایف من» everywhere (sidebar, palette, breadcrumbs source, page title, dashboard card)
- Roles now: مدیر / کارشناس ارشد (NEW) / کارشناس (renamed from کاربر); persisted end-to-end (DTO → SQLite → JWT → UI chips/filter/dialog)
- Runtime: backend on :3001 (no-watch mode; restart manually after backend edits), frontend on :3000 spawned by FrontendSupervisor
- Demo: admin@bpms.local/admin123, john|jane|bob@bpms.local/user123, NEW sara@bpms.local/user123 (کارشناس ارشد)
