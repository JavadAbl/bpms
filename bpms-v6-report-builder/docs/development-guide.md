...
|---|---|---|
| `test-v4-features.cjs` | **v4 sign-off**: starter restriction (403/200 paths, admin bypass, empty=all, 400/403 guardrails), کارتابل pending-only (mine + admin), condition engine contract + full Annual routing, create-with-starters | creates + leaves «فرآیند تست v4 شروع محدود» (idempotent by name; rerun reuses the leftover because terminated instances block deletion) |
