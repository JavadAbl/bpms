...
### 2.0 Process starters — the START event's assignment (v4)

Every process additionally has a **starter restriction** — the assignment of its start
 event, configured in the BPMN designer (right-click the start event → «تعیین شروع‌کنندگان»,
 or the «شروع: …» header chip). Stored as `ProcessStarter (processId, userId UNIQUE)` rows:

| Starter set | Who may start instances |
|---|---|
| empty (default) | every authenticated user |
| non-empty | only the listed users **+ every ADMIN** (god-mode so flows never dead-end) |

- Enforced server-side in `POST /process-instances` — non-starters get **403** «شما مجاز به شروع این فرآیند نیستید…».
- API: `PUT /processes/:id/starters {userIds}` replaces the set (empty = lift restriction); `POST /processes` accepts `starterIds[]` (created together with the process on designer-save).
- The «شروع فرآیند» dialog only lists processes the current user may start («محدود» badge on restricted ones).
- Not versioned (it is process-level config, not BPMN XML); running instances are unaffected by restriction changes.
...
- **کارتابل = received tasks only (v4)**: `/tasks/mine` (and the admin `/tasks`) list `PENDING` tasks exclusively. Completing a task removes it from the inbox immediately.
- **سوابق کارتابل = participated tasks (v4)**: `GET /tasks/participated` lists the caller's once-received tasks that have since passed — `COMPLETED` by them (including position/open tasks they completed or claimed) or `CANCELLED` when the instance ended while the task sat claimed in their inbox. It is strictly disjoint from the pending inbox; the UI route is `/tasks/participated`. Full history across all users stays visible on the instance timeline (گزارش فرآیندها / instance detail).
...
- **Dashboard scoping (v5):** every KPI on the landing dashboard is the caller's own data — pending tasks as in `/tasks/mine`, instances as in `/process-instances/mine`, and `activeProcesses` counts only ACTIVE processes the caller may start (empty starter set, or on the starter list; admins see the global count). The «گزارش فرآیندها» page loads `/process-instances/mine` directly for non-admins.
...
- **Reports (v6) resolve variables with the SAME merge semantics** — `ReportsService` re-implements the prefill-chain merge (documented identical: chronological submissions, `field.variable || field.name`, later wins) so report values can never disagree with what runtime forms display. Report variable filters are evaluated on the merged variables, independent of which columns the report displays. See `docs/v6-changelog.md`.
