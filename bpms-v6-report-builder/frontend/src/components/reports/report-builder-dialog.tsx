...
/**
 * Report builder dialog (v6) — the admin "no-code" report designer.
 *
 * A report is pure declarative configuration: pick a process, then select
 * columns from its instance data fields and process variables, optionally
 * add STATUS / DATE_RANGE / VARIABLE filters, preview the live result and
 * save. The same execution endpoint runs the preview and the saved report,
 * so «پیش‌نمایش» can never lie.
 */
export function ReportBuilderDialog({ open, onOpenChange, report, onSaved }: Props) {
  const isEdit = !!report;
  ...
  // Reset + initialize state whenever the dialog opens
  useEffect(() => {
    if (!open) return;
    setError('');
    setPreview(null);
  ...
  const buildFilters = (): ReportFilterConfig[] => {
      ...
      <DialogContent className="max-w-3xl max-h-[92vh] overflow-y-auto">
        <DialogHeader className="pe-12">
          <DialogTitle className="flex items-center gap-3 text-lg">
            ...
            {isEdit ? t.editReport : t.createReport}
          </DialogTitle>
          <DialogDescription>{t.reportBuilderHint}</DialogDescription>
        </DialogHeader>
        ...
        <section className="space-y-3">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div className="space-y-1.5">
              ...
              <Select
                defaultValue={report?.processId || undefined}
                onValueChange={(v) => {
                  setProcessId(v);
                  setColumns([]);
                  setStatusSet(new Set());
        ...
        {preview && (
          <section className="space-y-2 pt-2 border-t border-border/70">
            ...
            <div className="rounded-xl border border-border/70 overflow-x-auto max-h-72 overflow-y-auto">
