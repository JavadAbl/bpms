...
async function main() {
  ...
  console.log('🧹 Cleared: submissions, tasks, assignments, reports, starters, instances, variables, processes, forms, categories, positions, departments (users preserved)');
...
<bpmn:definitions xmlns:bpmn="http://www.omg.org/spec/BPMN/20100524/MODEL" xmlns:bpmndi="http://www.omg.org/spec/BPMN/20100524/DI" xmlns:dc="http://www.omg.org/spec/DD/20100524/DC" xmlns:di="http://www.omg.org/spec/DD/20100524/DI" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" id="Definitions_LeaveFa" targetNamespace="http://bpmn.io/schema/bpmn">
  ...
  // -----------------------------------------------------------------------
  // 7) Seeded report definitions (v6 report builder)
  //    Declarative configs — column keys follow the field:…/var:… convention
  // -----------------------------------------------------------------------
  await prisma.reportDefinition.createMany({
    data: [
      {
        name: 'گزارش مرخصی‌ها',
        description: 'تمامی درخواست‌های مرخصی با نوع، تاریخ‌ها و تصمیم نهایی',
        processId: process.id,
        createdById: admin.id,
        columns: JSON.stringify([
          ...
          { key: 'field:completedAt', source: 'INSTANCE', fieldKey: 'completedAt' },
          { key: 'var:employeeName', source: 'VARIABLE', fieldKey: 'employeeName' },
          { key: 'var:leaveType', source: 'VARIABLE', fieldKey: 'leaveType' },
          { key: 'var:startDate', source: 'VARIABLE', fieldKey: 'startDate' },
          { key: 'var:endDate', source: 'VARIABLE', fieldKey: 'endDate' },
          { key: 'var:decision', source: 'VARIABLE', fieldKey: 'decision' },
        ]),
        filters: JSON.stringify([]),
      },
      {
        name: 'مرخصی‌های تکمیل‌شده',
        description: 'درخواست‌های به‌انجام‌رسیده همراه با مدت اجرا و نظر مدیر',
        processId: process.id,
        createdById: admin.id,
        columns: JSON.stringify([
          { key: 'field:startedBy', source: 'INSTANCE', fieldKey: 'startedBy' },
          ...
          { key: 'var:leaveType', source: 'VARIABLE', fieldKey: 'leaveType' },
          { key: 'var:comment', source: 'VARIABLE', fieldKey: 'comment' },
        ]),
        filters: JSON.stringify([{ type: 'STATUS', statuses: ['COMPLETED'] }]),
      },
    ],
  });
  console.log('📊 2 report definitions seeded (گزارش مرخصی‌ها، مرخصی‌های تکمیل‌شده)');
