...
function CreateDeptDialog({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  const { values, setValue, errorFor, validate } = useZodForm(departmentSchema, {
    name: '',
    description: '',
  });
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await departmentsApi.create(parsed);
...
function CreatePosDialog({ deptId, onClose, onCreated }: { deptId: string; onClose: () => void; onCreated: () => void }) {
  const { toast } = useToast();
  const [saving, setSaving] = useState(false);
  const { values, setValue, errorFor, validate } = useZodForm(positionSchema, {
    name: '',
    description: '',
  });
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await positionsApi.create(deptId, parsed);
